...
int LabelTable::labelIsDefined(const std::string& token, int value)
{
	...
	if (index == -1) {
		...
		if (label_table[index].value == -1) {
			...
			std::cout << "Label " << addQuotes(token) << " is defined more than once - first occurrence used" << std::endl;
		}
	}
...
int LabelTable::labelIsOperand(const std::string& token)
{
	int index{ lookupLabel(token) };
...
std::string LabelTable::getName(int index) const
{
...
std::string LabelTable::instructionIndexToLabels(int index) const
{
	std::string result;
...
{
	if (num_labels == 0) {
		std::cout << std::endl << "The label table is empty" << std::endl;
	}
	else {
		std::cout << std::endl << "Label table: [index | label name | label value]" << std::endl << std::endl;
		for (int i{ 0 }; i < num_labels; i++) {
			std::cout << std::right << std::setw(8)  << i << "    "
				 << std::left  << std::setw(24) << addQuotes(label_table[i].name)
				 << std::right << std::setw(8)  << label_table[i].value << std::endl;
		}
	}
}
...
int LabelTable::lookupLabel(const std::string& token) const
{
	int index{ -1 };
...
int LabelTable::addLabel(const std::string& token, int value)
{
	int index{ -1 };
	...
	if (num_labels < MAX_LABELS) {
		...
		std::cout << "MAX_LABELS (" << MAX_LABELS << ") exceeded" << std::endl;
	}

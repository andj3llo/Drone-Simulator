...
void FlightPlanParse::parseLine(const std::string& line)
{
	...
	ParseStates myparseState = ParseStates::WHITE_SPACE;


	const int MAX_TOKENS{ 3 };		// Maximum number of tokens allowed
	std::string tokens[MAX_TOKENS];		// Array that stores the tokens found on the line
	int num_tokens{ 0 };			// Current number of tokens found on the line
	std::string token;					// Accumulates characters forming a token
	bool complete_token{ false };	// Set to true when a complete token has been formed

	const std::size_t n{ line.length() };
	...
	for (std::size_t i{ 0 }; i < n; i++) {
		...
		case ParseStates::WHITE_SPACE:
			if (c == BLANK || c == TAB)
			{
				myparseState = ParseStates::WHITE_SPACE;
			}
			else if (c == DRONE_START) {
				token += DRONE_START;
				myparseState = ParseStates::DRONE_COMMAND;
			}
			else if (c == COMMENT_START) {
				myparseState = ParseStates::COMMENT;
			}
			else {
				token += c;
				myparseState = ParseStates::OTHER_TOKEN;
			}
			break;
		...
		if (complete_token) {
			if (num_tokens < MAX_TOKENS) {
				...
				std::cout << "Too many operand(s) in line " << addQuotes(line) << std::endl;
				parse_success = false;
			}
			num_tokens++;
			token.clear();
	...
	if (num_tokens > 0) {
		if (!addLabelOrInstruction(tokens)) {
			...
			std::cout << "in line " << addQuotes(line) << std::endl;
			parse_success = false;
		}
	}
}
...
void FlightPlanParse::displayInstructions() const
{
	...
	if (num_instructions == 0) {
		std::cout << std::endl << "The instruction table is empty" << std::endl;
	}
	else {
		std::cout << std::endl << "Reconstructed instruction table:" << std::endl << std::endl;
		for (int i{ 0 }; i < num_instructions; i++) {
			std::string labels{ label_table.instructionIndexToLabels(i) };
			if (labels.length() > 0) {
				std::cout << labels;
			}
			if (validInstructionOperands(i)) {
				std::cout << "            " << indexToInstructionLine(i) << std::endl;
			}
		}
	}
}
...
bool FlightPlanParse::addLabelOrInstruction(std::string tokens[])
{
	bool valid_tokens{ false };

	if (isLabelDefinition(tokens[0])) {
		if (tokens[1].empty() && tokens[2].empty()) {
			std::size_t i{ tokens[0].rfind(':') };
...
std::string FlightPlanParse::indexToInstructionLine(int index) const
{
...
bool FlightPlanParse::validInstructionOperands(int index) const
{
	...
	if (!valid_operands) {
		std::cout << opcodeToString(instruction.opcode) << " instruction at location " << index
			 << " - program execution may fail" << std::endl;
	}
